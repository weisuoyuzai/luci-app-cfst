...
- **GitHub 加速前缀**：国内访问 GitHub Releases 可能较慢，可在此填入一个反代前缀（如 `https://gh-proxy.com/`），留空则直连 GitHub。公共加速服务可用性会变化，用不了时请自行更换
...
会自动移除 cron 任务；`/etc/config/cfst`（配置）、`/usr/share/cfst/bin/`（下载好的 cfst 程序）以及 `/usr/share/cfst/state/`（日志、上次运行状态、测速结果、下载状态，会持久保存，日志会自动限制大小）不会被删除，如需彻底清理请手动 `rm -rf /usr/share/cfst/bin /usr/share/cfst/state`。仅 pid 锁文件存放在 `/tmp/cfst/`（tmpfs），重启后自动清空，无需手动清理。
